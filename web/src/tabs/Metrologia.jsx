const uncertainty = [
  ['Alicate Amperimétrico', 'Calibração', 'B', 'Normal', '0,120', '1,000', '0,014400'],
  ['Resolução', 'Resolução', 'B', 'Retangular', '0,029', '1,000', '0,000841'],
...
const certs = [
  ['CLP-001', 'Alicate Amperimétrico', '12345678', 'CAL-2024-0456', '31/05/2024', '31/05/2025', 'Válido'],
  ['VAT-002', 'Voltímetro CA', '87654321', 'CAL-2024-0312', '15/05/2024', '15/05/2025', 'Válido'],
  ['WAT-003', 'Wattímetro', 'A1B2C3D4', 'CAL-2024-0288', '10/05/2024', '10/05/2025', 'Válido'],
  ['TC-005', 'TC 600/5 A', 'TC6005-01', 'CAL-2024-0180', '28/04/2024', '28/04/2025', 'Válido'],
...
export default function Metrologia() {
  return (
    <div style={{ minHeight: 1180, display: 'grid', gridTemplateColumns: '1.2fr 1fr 0.85fr 0.85fr', gridTemplateRows: '260px 300px 300px 240px', gap: 14, padding: 14, overflow: 'visible' }}>
      <div className="panel">
        <div className="panel__head">Orçamento de Incerteza da Medição</div>
        <table className="tbl">
      ...
      <div className="panel">
        ...
        <table className="tbl">
          <thead><tr><th>ID</th><th>Instrumento</th><th>Certificado</th><th>Data</th><th>Próx.</th><th>Status</th></tr></thead>
          <tbody>{certs.map(r => <tr key={r[0]}><td>{r[0]}</td><td>{r[1]}</td><td>{r[3]}</td><td>{r[4]}</td><td>{r[5]}</td><td><span className={r[6] === 'Válido' ? 'badge badge-green' : 'badge badge-yellow'}>{r[6]}</span></td></tr>)}</tbody>
        </table>
      </div>
      ...
      <DataPanel title="Repetibilidade e Reprodutibilidade (Gage R&R)" rows={[['Repetibilidade', '27', '0,00362', '18,4%'], ['Reprodutibilidade', '2', '0,00674', '34,2%'], ['Interação', '54', '0,00221', '11,2%'], ['Peça', '9', '0,00781', '39,7%']]} />
      ...
      <StepPanel title="Procedimentos de Medição" steps={['Verificar validade do certificado.', 'Inspecionar instrumento e acessórios.', 'Confirmar condições ambientais.', 'Configurar faixa e função.', 'Realizar medição conforme norma.', 'Registrar resultados e incertezas.']} />
      <DataPanel title="Tabela de Calibração - Referência vs. Medido" rows={[['0,00', '0,02', '0,02', '0,27', 'Conforme'], ['50,00', '50,05', '0,05', '0,27', 'Conforme'], ['100,00', '100,10', '0,10', '0,27', 'Conforme'], ['200,00', '200,24', '0,12', '0,27', 'Conforme'], ['1000,0', '1002,10', '0,21', '0,27', 'Conforme']]} />
      ...
      <StepPanel title="Bloqueio e Etiquetagem (LOTO)" steps={['Desligar', 'Isolar', 'Bloquear', 'Etiquetar', 'Verificar ausência de tensão']} />
      <DataPanel title="Verificação de Aterramento" rows={[['Método', 'Queda de Potencial'], ['Resistência Medida', '0,32 Ω'], ['Limite Recomendado', '≤ 10 Ω'], ['Status', 'Conforme']]} />
      <DataPanel title="Classes de Segurança (CAT)" rows={[['CAT IV', '1000 V', 'Entrada de serviço'], ['CAT III', '600 V', 'Distribuição'], ['CAT II', '300 V', 'Cargas'], ['CAT I', '150 V', 'Eletrônica']]} />
      <RiskMatrix />
      <div className="panel">
...
function PolarityDiagram() {
  return <div style={{ textAlign: 'center' }}><b>Marcação e Polaridade</b><svg viewBox="0 0 220 130" style={{ width: '100%', height: 115 }}><line x1="35" y1="35" x2="190" y2="35" stroke="#111827" /><line x1="35" y1="92" x2="190" y2="92" stroke="#111827" /><circle cx="35" cy="35" r="6" fill="#ef4444" /><circle cx="35" cy="92" r="6" fill="#ef4444" /><text x="28" y="24">P1</text><text x="180" y="24">P2</text><text x="28" y="82">S1</text><text x="180" y="82">S2</text></svg></div>
}
