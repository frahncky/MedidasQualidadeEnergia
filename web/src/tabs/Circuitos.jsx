...
const LOAD_TYPES = [
  ...
  { id: 'LC',  label: 'LC — Indutiva-Capacitiva',  color: '#059669', fields: ['L', 'C'] },
...
// Returns complex impedance [Re, Im] for a single branch (includes LC)
function compZ(branch, f) {
  const t   = branch.type || 'R'
  const R   = parseNum(branch.R) || 0
  const L   = parseNum(branch.L) || 0
  const C   = parseNum(branch.C) || 0
  const hasR = ['R','RL','RC','RLC','M'].includes(t)
  const hasL = ['L','LC','RL','RLC','M'].includes(t)
  const hasC = ['C','LC','RC','RLC'].includes(t)
  const XL  = hasL ? 2*Math.PI*f*L : 0
  const XC  = hasC && C > 0 ? 1/(2*Math.PI*f*C) : 0
  return [hasR ? R : 0, XL - XC]
}

// Parallel combination of complex impedances via admittance sum
function parallelZ(Zs) {
  let Yr = 0, Yi = 0
  for (const Z of Zs) {
    const m2 = Z[0]**2 + Z[1]**2 || 1e-30
    Yr += Z[0]/m2
    Yi -= Z[1]/m2
  }
  return cx.div([1,0], [Yr, Yi])
}

// CC circuit: blocks in series, branches within block in parallel (pure resistance)
function calcBuiltCC(src, blocks) {
  const V   = parseNum(src.V)  || 12
  const rA  = parseNum(src.rA) || 0
  const RV  = parseNum(src.RV) || 1e9

  const blockRes = blocks.map(block => {
    const Rs      = block.branches.map(b => Math.max(parseNum(b.R) || 0, 1e-9))
    const R_block = Rs.length === 1 ? Rs[0] : 1 / Rs.reduce((s,r) => s+1/r, 0)
    return { ...block, Rs, R_block }
  })

  const R_load  = blockRes.reduce((s,b) => s+b.R_block, 0) || 1e-9
  const R_total = R_load + rA
  const I_src   = V / R_total
  const V_load  = I_src * R_load

  const detailed = blockRes.map(b => {
    const V_b  = I_src * b.R_block
    return { ...b, V_block: V_b, branchIs: b.Rs.map(R => V_b/R) }
  })

  const crossover = Math.sqrt(rA * RV)
  return { V, rA, RV, R_load, R_total, I_src, V_load, blocks: detailed,
    crossover, mc_better: R_load < crossover }
}

// CA single-phase: blocks in series, branches within block in parallel (complex Z)
function calcBuiltCA(src, blocks) {
  const Vs = parseNum(src.Vs) || 127
  const f  = parseNum(src.f)  || 60

  const blockRes = blocks.map(block => {
    const Zs      = block.branches.map(b => compZ(b, f))
    const Z_block = Zs.length === 1 ? Zs[0] : parallelZ(Zs)
    return { ...block, Zs, Z_block }
  })

  let Z_total = [0, 0]
  for (const b of blockRes) Z_total = cx.add(Z_total, b.Z_block)

  const Zmag  = cx.mag(Z_total) || 1e-10
  const phi   = cx.arg(Z_total)
  const I_tot = cx.div([Vs, 0], Z_total)
  const I_mag = cx.mag(I_tot)

  const detailed = blockRes.map(b => {
    const V_b   = cx.mul(I_tot, b.Z_block)
    const V_mag = cx.mag(V_b)
    const V_ang = cx.arg(V_b)
    const I_br  = b.Zs.map(Z => cx.div(V_b, Z))
    return { ...b, V_block: V_b, V_mag, V_ang, I_br, I_bmag: I_br.map(I => cx.mag(I)) }
  })

  const FP  = Math.cos(phi)
  const P   = Vs * I_mag * Math.abs(FP)
  const Q   = Vs * I_mag * Math.abs(Math.sin(phi))
  const S   = Vs * I_mag
  const nat = phi > 0.001 ? 'Indutivo' : phi < -0.001 ? 'Capacitivo' : 'Resistivo'

  return { Vs, f, Z_total, Zmag, phi, I_tot, I_mag, FP, P, Q, S, nat, blocks: detailed }
}

// CA 3-phase: parallel loads on a single bus with optional line resistance
function calcBusTri(src, loads) {
  const VL    = parseNum(src.VL)    || 13800
  const f     = parseNum(src.f)     || 60
  const Rline = parseNum(src.Rline) || 0
  const V_ph  = VL / Math.sqrt(3)

  const loadRes = loads.map(load => {
    let Z_Y = phaseZ(load.type, load, f, VL)
    if (load.ligacao === 'D') Z_Y = [Z_Y[0]/3, Z_Y[1]/3]
    const Zmag   = cx.mag(Z_Y) || 1e-10
    const phi    = cx.arg(Z_Y)
    const I_ph   = V_ph / Zmag
    const I_line = load.ligacao === 'D' ? Math.sqrt(3)*I_ph : I_ph
    const FP     = Math.cos(phi)
    const P      =  3 * V_ph * I_ph * Math.abs(FP)
    const Q_sgn  = (3 * V_ph * I_ph * Math.abs(Math.sin(phi))) * (phi > 0 ? 1 : -1)
    const S      =  3 * V_ph * I_ph
    const nat    = phi > 0.001 ? 'Ind' : phi < -0.001 ? 'Cap' : 'R'
    return { ...load, Z_Y, Zmag, phi, I_ph, I_line, FP, P, Q_sgn, Q: Math.abs(Q_sgn), S, nat }
  })

  // Sum admittances for total bus impedance
  let Yr = 0, Yi = 0
  for (const lr of loadRes) {
    const m2 = lr.Z_Y[0]**2 + lr.Z_Y[1]**2 || 1e-30
    Yr += lr.Z_Y[0]/m2; Yi -= lr.Z_Y[1]/m2
  }
  const Z_tot    = cx.div([1,0], [Yr||1e-10, Yi])
  const phi_tot  = cx.arg(Z_tot)
  const I_tot_ph = V_ph * Math.sqrt(Yr**2 + Yi**2)
  const FP_tot   = Math.cos(phi_tot)
  const P_tot    = loadRes.reduce((s,l) => s+l.P, 0)
  const Q_tot    = loadRes.reduce((s,l) => s+l.Q_sgn, 0)
  const S_tot    = Math.sqrt(P_tot**2 + Q_tot**2)

  const V_drop = Math.sqrt(3) * Rline * I_tot_ph
  const V_bus  = VL - V_drop
  const Ploss  = 3 * Rline * I_tot_ph**2
  const vd_pct = VL > 0 ? (V_drop/VL)*100 : 0

  return { VL, V_ph, f, Rline, V_drop, V_bus, Ploss, vd_pct,
    I_tot_ph, phi_tot, FP_tot, P_tot, Q_tot: Math.abs(Q_tot), Q_tot_sgn: Q_tot, S_tot,
    loads: loadRes }
}
...
function brLabelShort(br) {
  const t = br.type || 'R'
  const R = parseNum(br.R) || 0
  const L = parseNum(br.L) || 0
  const C = parseNum(br.C) || 0
  const p = []
  if (['R','RL','RC','RLC','M'].includes(t) && R) p.push(`${R}Ω`)
  if (['L','LC','RL','RLC','M'].includes(t) && L) p.push(`${L}H`)
  if (['C','LC','RC','RLC'].includes(t)    && C) p.push(`${(C*1000).toFixed(3).replace('.',',')}mF`)
  return p.join(' ')
}

function CircuitDiagramSVG({ blocks, isCA, result, src }) {
  const H = 150, r1 = 12, r2 = H - 12, cY = H / 2
  const BPB = 64, SRC = 56
  const blkWs = blocks.map(b => Math.max(b.branches.length * BPB, 72))
  const totalW = SRC + blkWs.reduce((s,w)=>s+w, 0) + 28
  const TMAP = Object.fromEntries(LOAD_TYPES.map(t => [t.id, t]))

  function CompSym({ bxc, y1c, y2c, type, label, color }) {
    const mh = 38, mw = 50, sy = (y1c+y2c)/2
    return (
      <>
        <line x1={bxc} y1={y1c} x2={bxc} y2={sy-mh/2} stroke="#475569" strokeWidth={1.5}/>
        <rect x={bxc-mw/2} y={sy-mh/2} width={mw} height={mh}
          fill={`${color}18`} stroke={color} strokeWidth={2} rx={4}/>
        <text x={bxc} y={sy-5} textAnchor="middle" fontSize={10} fontWeight="700" fill={color}>{type}</text>
        <text x={bxc} y={sy+9} textAnchor="middle" fontSize={7} fill={color}>{label}</text>
        <line x1={bxc} y1={sy+mh/2} x2={bxc} y2={y2c} stroke="#475569" strokeWidth={1.5}/>
      </>
    )
  }
  ...
  return (
    <svg viewBox={`0 0 ${totalW} ${H}`} style={{ width:'100%', height:H }}>
      <line x1={24} y1={r1} x2={totalW-8} y2={r1} stroke="#475569" strokeWidth={2}/>
      <line x1={24} y1={r2} x2={totalW-8} y2={r2} stroke="#475569" strokeWidth={2}/>
      <line x1={totalW-8} y1={r1} x2={totalW-8} y2={r2} stroke="#475569" strokeWidth={2}/>
      <circle cx={24} cy={cY} r={20} fill="var(--c-surface)" stroke="#334155" strokeWidth={2}/>
      {isCA
        ? <text x={24} y={cY+5} textAnchor="middle" fontSize={13}>~</text>
        : <text x={24} y={cY+4} textAnchor="middle" fontSize={10} fontWeight="700">{parseNum(src?.V)||12}V</text>
      }
      <line x1={24} y1={cY-20} x2={24} y2={r1} stroke="#475569" strokeWidth={2}/>
      <line x1={24} y1={cY+20} x2={24} y2={r2} stroke="#475569" strokeWidth={2}/>
      {isCA && <text x={24} y={H-2} textAnchor="middle" fontSize={7} fill="#1d4ed8">
        {src?.Vs}V {src?.f}Hz</text>}
      {blocks.map((block, bi) => {
        const bw = blkWs[bi]
        const bxStart = curX; curX += bw
        const brW = bw / block.branches.length
        const isP = block.branches.length > 1
        const blkR = result?.blocks?.[bi]
        return (
          <g key={block.id}>
            <text x={bxStart+bw/2} y={r1-1} textAnchor="middle" fontSize={7.5} fill="#94a3b8">
              B{bi+1}{isP?' ∥':''}
            </text>
            {blkR && (
              <text x={bxStart+bw/2} y={H-1} textAnchor="middle" fontSize={7} fill="#64748b">
                {isCA ? `V=${fmt(blkR.V_mag,1)}V` : `V=${fmt(blkR.V_block,1)}V`}
              </text>
            )}
            {isP && <>
              <line x1={bxStart+brW*0.3} y1={r1+1} x2={bxStart+bw-brW*0.3} y2={r1+1} stroke="#94a3b8" strokeWidth={1.5}/>
              <line x1={bxStart+brW*0.3} y1={r2-1} x2={bxStart+bw-brW*0.3} y2={r2-1} stroke="#94a3b8" strokeWidth={1.5}/>
            </>}
            {block.branches.map((br, j) => {
              const bxc = bxStart + (j+0.5)*brW
              const clr = isCA ? (TMAP[br.type]?.color ?? '#334155') : '#1d4ed8'
              const lbl = isCA ? brLabelShort(br) : `${parseNum(br.R)||0}Ω`
              const I_b = isCA ? blkR?.I_bmag?.[j] : blkR?.branchIs?.[j]
              return (
                <g key={br.id}>
                  <CompSym bxc={bxc} y1c={isP?r1+2:r1} y2c={isP?r2-2:r2}
                    type={isCA?br.type:'R'} label={lbl} color={clr}/>
                  {I_b !== undefined && (
                    <text x={bxc+28} y={cY+3} textAnchor="start" fontSize={7} fill="#64748b">
                      {fmt(I_b,3)}A
                    </text>
                  )}
                </g>
...
function PhasorBuilderSVG({ result }) {
  const W = 400, H = 190, ox = 110, oy = H/2
  const { I_mag, phi, Vs, blocks } = result
  const scV = 70 / (Vs || 1)
  const blkColors = ['#dc2626','#16a34a','#9333ea','#d97706','#0284c7','#ea580c']

  function Arr({ dx, dy, color, label, dashed }) {
    const len = Math.sqrt(dx*dx+dy*dy); if (len < 4) return null
    const ux = dx/len, uy = dy/len
    const x2 = ox+dx, y2 = oy+dy
    const hx = x2-7*ux, hy = y2-7*uy
    return (
      <g>
        <line x1={ox} y1={oy} x2={hx} y2={hy} stroke={color} strokeWidth={dashed?1.5:2}
          strokeDasharray={dashed?'5 3':undefined}/>
        <polygon points={`${x2},${y2} ${hx-4*uy},${hy+4*ux} ${hx+4*uy},${hy-4*ux}`} fill={color}/>
        <text x={x2+10*ux} y={y2+10*uy+4} textAnchor="middle" fontSize={9} fill={color} fontWeight="700">{label}</text>
      </g>
    )
  }
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:'100%', height:'calc(100% - 38px)' }}>
      ...
      <Arr dx={65} dy={0} color="#ea580c" label={`I=${fmt(I_mag,3)}A`}/>
      <Arr dx={scV*Vs*Math.cos(-phi)} dy={scV*Vs*Math.sin(-phi)} color="#1d4ed8" label={`Vs=${fmt(Vs,1)}V`}/>
      {blocks?.map((b, i) => {
        if (!b.V_block) return null
        return (
          <Arr key={b.id}
            dx={scV*b.V_mag*Math.cos(-b.V_ang)} dy={scV*b.V_mag*Math.sin(-b.V_ang)}
            color={blkColors[i%blkColors.length]} label={`V${i+1}=${fmt(b.V_mag,1)}V`} dashed/>
        )
      })}
      {Math.abs(phi) > 0.02 && (() => {
        const r = 28, ex = ox+r*Math.cos(-phi), ey = oy+r*Math.sin(-phi), sw = phi>0?0:1
        return <>
          <path d={`M ${ox+r} ${oy} A ${r} ${r} 0 0 ${sw} ${ex} ${ey}`}
            fill="none" stroke="#94a3b8" strokeWidth="1" strokeDasharray="3 2"/>
          <text x={ox+r*Math.cos(-phi/2)+12} y={oy-r*0.5*Math.sin(-phi/2)} fontSize="9" fill="#64748b">
            φ={fmt(phi*180/Math.PI,1)}°</text>
        </>
      })()}
      <text x={W/2} y={H-3} textAnchor="middle" fontSize="11" fontWeight="700" fill="#334155">
        {result.nat}  ·  FP = {fmt(result.FP,4)}  ·  φ = {fmt(result.phi*180/Math.PI,2)}°
      </text>
    </svg>
  )
}

function CCMeasurementPanel({ result }) {
  if (!result) return null
  const { R_load, R_total, I_src, V_load, crossover, mc_better, rA, RV } = result
  return (
    <div style={{ padding:'12px', overflow:'auto', height:'100%' }}>
      <Section title="Circuito">
        <Result label="R_eq carga"    value={`${fmt(R_load,3)} Ω`}/>
        <Result label="R_total c/r_A" value={`${fmt(R_total,3)} Ω`}/>
        <Result label="I fonte"        value={`${fmt(I_src,5)} A`}/>
        <Result label="V carga"        value={`${fmt(V_load,3)} V`}/>
        <Result label="P carga"        value={`${fmt(I_src**2*R_load,2)} W`}/>
      </Section>
      {crossover < 1e8 && (
        <Section title="Posição dos Instrumentos">
          <Result label="r_A"             value={`${fmt(rA,3)} Ω`}/>
          <Result label="R_V"             value={parseNum(RV)<1e8?`${fmt(parseNum(RV),0)} Ω`:'∞'}/>
          <Result label="R★ = √(r_A·R_V)" value={`${fmt(crossover,2)} Ω`}/>
          <div style={{ marginTop:8, padding:'8px 12px', borderRadius:6, fontWeight:700,
            background: mc_better ? '#dcfce7' : '#e0f2fe',
            color: mc_better ? '#16a34a' : '#0369a1' }}>
            R_eq {mc_better?'<':'>'} R★ → {mc_better?'Montagem CURTA':'Montagem LONGA'}
          </div>
          <div style={{ fontSize:10, color:'var(--c-text-muted)', marginTop:6 }}>
            {mc_better
              ? 'Voltímetro externo: mede V exato, erro em I por r_A.'
              : 'Voltímetro interno: mede I exato, erro em V pela corrente em R_V.'}
          </div>
        </Section>
      )}
    </div>
...
function CircuitBuilder({ isCA }) {
  const [src, setSrc] = useState(
    isCA ? { Vs:'127', f:'60' } : { V:'12', rA:'0,1', RV:'10000' }
  )
  const [blocks, setBlocks] = useState([
    { id:1, branches:[{ id:1, type:'R', R:'100', L:'0,05', C:'0,001' }] }
  ])
  const [nType, setNType] = useState('R')
  const [nR, setNR] = useState('100')
  const [nL, setNL] = useState('0,05')
  const [nC, setNC] = useState('0,001')

  const sSrc = (k,v) => setSrc(p => ({...p, [k]:v}))
  const result = useMemo(
    () => isCA ? calcBuiltCA(src, blocks) : calcBuiltCC(src, blocks),
    [isCA, src, blocks]
  )

  const mkBr = () => ({ id:Date.now(), type:nType, R:nR, L:nL, C:nC })
  const addSeries   = () => setBlocks(p => [...p, { id:Date.now(), branches:[mkBr()] }])
  const addParallel = () => setBlocks(p =>
    p.map((b,i) => i < p.length-1 ? b : { ...b, branches:[...b.branches, mkBr()] })
  )
  const removeBlock  = bid => setBlocks(p => p.length>1 ? p.filter(b=>b.id!==bid) : p)
  const removeBranch = (bid, brid) => setBlocks(p => {
    const next = p.map(b => {
      if (b.id!==bid) return b
      const nb = b.branches.filter(br=>br.id!==brid)
      return nb.length ? {...b, branches:nb} : null
    }).filter(Boolean)
    return next.length ? next : p
  })
  const updBranch = (bid, brid, k, v) => setBlocks(p => p.map(b => b.id!==bid ? b : {
    ...b, branches: b.branches.map(br => br.id!==brid ? br : {...br, [k]:v})
  }))

  const nlt = LOAD_TYPES.find(t=>t.id===nType) ?? LOAD_TYPES[0]

  return (
    <div style={{ display:'grid', gridTemplateColumns:'210px 1fr 240px', gap:12, padding:12,
      flex:'1 1 auto', minHeight:0, overflow:'auto' }}>

      {/* Left */}
      <div className="panel" style={{ display:'flex', flexDirection:'column', minHeight:0 }}>
        <div className="panel__head">{isCA ? 'CA Monofásico' : 'CC'}</div>
        <div style={{ flex:1, overflow:'auto', padding:'10px 12px' }}>

          <Section title="Fonte">
            {isCA ? <>
              <PField label="Tensão Vs"  unit="V"  value={src.Vs} onChange={v=>sSrc('Vs',v)}/>
              <PField label="Frequência" unit="Hz" value={src.f}  onChange={v=>sSrc('f',v)}/>
            </> : <>
              <PField label="Tensão V" unit="V" value={src.V}  onChange={v=>sSrc('V',v)}/>
              <PField label="r_A"      unit="Ω" value={src.rA} onChange={v=>sSrc('rA',v)}/>
              <PField label="R_V"      unit="Ω" value={src.RV} onChange={v=>sSrc('RV',v)}/>
            </>}
          </Section>
                ...
                {LOAD_TYPES.map(t => (
                  <button key={t.id} className="btn btn-sm"
                    style={{ fontSize:9, padding:'2px 6px',
                      background: nType===t.id ? t.color : 'transparent',
                      color: nType===t.id ? '#fff' : 'var(--c-text)',
                      border: `1px solid ${t.color}` }}
                    onClick={() => setNType(t.id)}>{t.id}</button>
                ))}
              </div>
            )}
            {(!isCA || nlt.fields.includes('R')) &&
              <PField label="R" unit="Ω" value={nR} onChange={setNR}/>}
            {isCA && nlt.fields.includes('L') &&
              <PField label="L" unit="H" value={nL} onChange={setNL}/>}
            {isCA && nlt.fields.includes('C') &&
              <PField label="C" unit="F" value={nC} onChange={setNC}/>}
            <div style={{ display:'flex', gap:6, marginTop:8 }}>
              <button className="btn btn-sm" style={{ flex:1, background:'#1d4ed8', color:'#fff' }}
                onClick={addSeries}>+ Série</button>
              <button className="btn btn-sm btn-ghost" style={{ flex:1 }}
                onClick={addParallel}>∥ Paralelo</button>
            </div>
          </Section>

          <Section title={`Blocos (${blocks.length})`}>
            {blocks.map((b, bi) => {
              const isP = b.branches.length > 1
              return (
                <div key={b.id} style={{ marginBottom:8, border:'1px solid var(--c-border)',
                  borderRadius:6, padding:'5px 8px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                    <span style={{ fontSize:10, fontWeight:700, color:'var(--c-primary)' }}>
                      B{bi+1} {isP?'(∥ paralelo)':'(— série)'}
                    </span>
                    <button style={{ fontSize:13, color:'#dc2626', background:'none', border:'none', cursor:'pointer' }}
                      onClick={() => removeBlock(b.id)}>×</button>
                  </div>
                  {b.branches.map(br => {
                    const blt = LOAD_TYPES.find(t=>t.id===br.type) ?? LOAD_TYPES[0]
                    const clr = isCA ? blt.color : '#1d4ed8'
                    return (
                      <div key={br.id} style={{ display:'flex', alignItems:'center', gap:3, marginBottom:4 }}>
                        <span style={{ color:clr, fontWeight:700, fontSize:10, minWidth:26 }}>
                          {isCA?br.type:'R'}
                        </span>
                        {(!isCA||blt.fields.includes('R')) &&
                          <input className="form-input" style={{ width:46, fontSize:10 }}
                            value={br.R} onChange={e=>updBranch(b.id,br.id,'R',e.target.value)}/>}
                        {isCA&&blt.fields.includes('L') &&
                          <input className="form-input" style={{ width:46, fontSize:10 }}
                            value={br.L} onChange={e=>updBranch(b.id,br.id,'L',e.target.value)}/>}
                        {isCA&&blt.fields.includes('C') &&
                          <input className="form-input" style={{ width:46, fontSize:10 }}
                            value={br.C} onChange={e=>updBranch(b.id,br.id,'C',e.target.value)}/>}
                        <button style={{ marginLeft:'auto', fontSize:12, color:'#94a3b8',
                          background:'none', border:'none', cursor:'pointer' }}
                          onClick={() => removeBranch(b.id, br.id)}>×</button>
                      </div>
                    )
                  })}
                </div>
            ...
            <button className="btn btn-ghost btn-sm" style={{ width:'100%', marginTop:2 }}
              onClick={() => setBlocks([{ id:Date.now(), branches:[{ id:1, type:'R', R:'100', L:'0,05', C:'0,001' }] }])}>
              Limpar circuito
            </button>
          </Section>
        </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:12, minHeight:0 }}>
        <div className="panel" style={{ flexShrink:0 }}>
          <div className="panel__head">Diagrama do Circuito</div>
          <div style={{ padding:'8px 12px', overflowX:'auto' }}>
            <CircuitDiagramSVG blocks={blocks} isCA={isCA} result={result} src={src}/>
          </div>
        </div>
        <div className="panel" style={{ flex:'1 1 auto', minHeight:170 }}>
          {isCA ? <>
            <div className="panel__head">Diagrama Fasorial — ref.: corrente I</div>
            <PhasorBuilderSVG result={result}/>
          </> : <>
            <div className="panel__head">Análise de Medição</div>
            <CCMeasurementPanel result={result}/>
          </>}
        </div>
      </div>
      ...
      <div className="panel" style={{ display:'flex', flexDirection:'column', minHeight:0 }}>
        ...
        <div style={{ flex:1, overflow:'auto', padding:'10px 12px' }}>
          {isCA ? <>
            <Section title={`Total — ${result.nat}`}>
              <Result label="|Z|"  value={`${fmt(result.Zmag,3)} Ω`}/>
              <Result label="Re Z" value={`${fmt(result.Z_total[0],3)} Ω`}/>
              <Result label="Im Z" value={`${fmt(result.Z_total[1],3)} Ω`}/>
              <Result label="φ"    value={`${fmt(result.phi*180/Math.PI,2)}°`}/>
              <Result label="I"    value={`${fmt(result.I_mag,4)} A`}/>
              <Result label="FP"   value={fmt(result.FP,4)} highlight={result.FP<0.92&&result.S>1}/>
            </Section>
            <Section title="Potências">
              <Result label="P" value={`${fmt(result.P,1)} W`}/>
              <Result label="Q" value={`${fmt(result.Q,1)} var (${result.nat})`}/>
              <Result label="S" value={`${fmt(result.S,1)} VA`}/>
            </Section>
            {result.blocks.map((b,i) => (
              <Section key={b.id} title={`Bloco ${i+1} ${b.branches.length>1?'(∥)':''}`}>
                <Result label="|Z|" value={`${fmt(cx.mag(b.Z_block),3)} Ω`}/>
                <Result label="|V|" value={`${fmt(b.V_mag,2)} V`}/>
                {b.branches.map((br,j) => (
                  <Result key={br.id} label={`I${b.branches.length>1?`_${br.type}${j+1}`:`_${br.type}`}`}
                    value={`${fmt(b.I_bmag[j],4)} A`}/>
                ))}
              </Section>
            ))}
          </> : <>
            <Section title="Circuito CC">
              <Result label="R_eq carga"    value={`${fmt(result.R_load,3)} Ω`}/>
              <Result label="R_total c/r_A" value={`${fmt(result.R_total,3)} Ω`}/>
              <Result label="I fonte"        value={`${fmt(result.I_src,5)} A`}/>
              <Result label="V carga"        value={`${fmt(result.V_load,3)} V`}/>
              <Result label="P carga"        value={`${fmt(result.I_src**2*result.R_load,2)} W`}/>
            </Section>
            {result.blocks.map((b,i) => (
              <Section key={b.id} title={`Bloco ${i+1} ${b.branches.length>1?'(∥)':''}`}>
                <Result label="R_eq" value={`${fmt(b.R_block,3)} Ω`}/>
                <Result label="V"    value={`${fmt(b.V_block,2)} V`}/>
                {b.branches.map((br,j) => (
                  <Result key={br.id} label={`I${j+1} (${parseNum(br.R)||0}Ω)`}
                    value={`${fmt(b.branchIs[j],4)} A`}/>
                ))}
              </Section>
            ))}
          </>}
...
function BusDiagramSVG({ loads, src }) {
  const H = 100, busY = 22, pad = 20
  const loadW = Math.max(60, Math.min(100, 380/(loads.length||1)))
  const totalW = pad*2 + 60 + loads.length*loadW
  return (
    <svg viewBox={`0 0 ${totalW} ${H}`} style={{ width:'100%', height:H }}>
      <circle cx={pad+20} cy={busY} r={18} fill="var(--c-surface)" stroke="#334155" strokeWidth={2}/>
      <text x={pad+20} y={busY+5} textAnchor="middle" fontSize={12}>~</text>
      <text x={pad+20} y={H-4} textAnchor="middle" fontSize={7.5} fill="#1d4ed8">3F {src.VL}V</text>
      <line x1={pad+38} y1={busY} x2={pad+60} y2={busY} stroke="#334155" strokeWidth={2}/>
      {parseNum(src.Rline)>0 && <>
        <rect x={pad+42} y={busY-8} width={14} height={16} fill="var(--c-surface)" stroke="#334155" rx={2}/>
        <text x={pad+49} y={busY+4} textAnchor="middle" fontSize={8}>R</text>
      </>}
      <line x1={pad+60} y1={busY} x2={pad+60+loads.length*loadW} y2={busY} stroke="#334155" strokeWidth={3}/>
      {loads.map((load, i) => {
        const lx  = pad + 60 + (i+0.5)*loadW
        const llt = TRI_LOAD_TYPES.find(t=>t.id===load.type) ?? TRI_LOAD_TYPES[1]
        return (
          <g key={load.id}>
            <line x1={lx} y1={busY} x2={lx} y2={busY+22} stroke="#334155" strokeWidth={1.5}/>
            <rect x={lx-22} y={busY+22} width={44} height={28} fill={`${llt.color}18`} stroke={llt.color} strokeWidth={2} rx={3}/>
            <text x={lx} y={busY+39} textAnchor="middle" fontSize={8.5} fontWeight="700" fill={llt.color}>{load.type}</text>
            <text x={lx} y={busY+51} textAnchor="middle" fontSize={7} fill={llt.color}>{load.ligacao}</text>
            <text x={lx} y={H-2} textAnchor="middle" fontSize={7} fill="#64748b">{load.label}</text>
          </g>
        )
      })}
    </svg>
...
function TriBusBuilder() {
  const [src, setSrc]    = useState({ VL:'13800', f:'60', Rline:'0,5' })
  const [loads, setLoads] = useState([
    { id:1, label:'Carga 1', type:'RL',  ligacao:'Y', R:'10',  L:'0,05', C:'0,001', Pn:'37', eta:'92', FPm:'0,87', Cuf:'50' },
    { id:2, label:'Carga 2', type:'M3F', ligacao:'Y', R:'5',   L:'0,03', C:'0,001', Pn:'75', eta:'94', FPm:'0,88', Cuf:'50' },
  ])
  const [nType, setNType] = useState('RL')
  const [nLig,  setNLig]  = useState('Y')
  const [nVals, setNVals] = useState({ R:'10', L:'0,05', C:'0,001', Pn:'37', eta:'92', FPm:'0,87', Cuf:'50' })
  const [nLabel, setNLabel] = useState('')

  const sSrc = (k,v) => setSrc(p => ({...p,[k]:v}))
  const result = useMemo(() => calcBusTri(src, loads), [src, loads])

  const addLoad = () => {
    const id    = Date.now()
    const label = nLabel || `Carga ${loads.length+1}`
    setLoads(p => [...p, { id, label, type:nType, ligacao:nLig, ...nVals }])
    setNLabel('')
  }
  const removeLoad = id   => setLoads(p => p.length>1 ? p.filter(l=>l.id!==id) : p)
  const updLoad    = (id,k,v) => setLoads(p => p.map(l=>l.id!==id?l:{...l,[k]:v}))
  const nlt = TRI_LOAD_TYPES.find(t=>t.id===nType) ?? TRI_LOAD_TYPES[1]

  return (
    <div style={{ display:'grid', gridTemplateColumns:'220px 1fr 260px', gap:12, padding:12,
      flex:'1 1 auto', minHeight:0, overflow:'auto' }}>

      {/* Left */}
      <div className="panel" style={{ display:'flex', flexDirection:'column', minHeight:0 }}>
        <div className="panel__head">Construtor de Cargas 3F</div>
        <div style={{ flex:1, overflow:'auto', padding:'10px 12px' }}>

          <Section title="Fonte 3F">
            <PField label="V linha"  unit="V"  value={src.VL}    onChange={v=>sSrc('VL',v)}/>
            <PField label="Freq."    unit="Hz" value={src.f}     onChange={v=>sSrc('f',v)}/>
            <PField label="R_linha"  unit="Ω"  value={src.Rline} onChange={v=>sSrc('Rline',v)}/>
          </Section>

          <Section title="Nova Carga">
            <input className="form-input" placeholder="Nome (opc.)" value={nLabel}
            ...
            <div style={{ display:'flex', gap:6, marginBottom:6 }}>
              {['Y','D'].map(l=>(
                <button key={l} className={`btn btn-sm${nLig===l?'':' btn-ghost'}`}
                  style={{ flex:1, background:nLig===l?'#1d4ed8':undefined, color:nLig===l?'#fff':undefined }}
                  onClick={()=>setNLig(l)}>{l==='Y'?'Y Estrela':'Δ Triângulo'}</button>
              ))}
            </div>
            <LoadParamFields lt={nType} prefix="" vals={nVals} onChange={(k,v)=>setNVals(p=>({...p,[k]:v}))}/>
            <button className="btn btn-sm" style={{ width:'100%', marginTop:8, background:'#1d4ed8', color:'#fff' }}
              onClick={addLoad}>+ Adicionar ao barramento</button>
          </Section>

          <Section title={`Cargas no barramento (${loads.length})`}>
            {loads.map((load, li) => {
              const llt = TRI_LOAD_TYPES.find(t=>t.id===load.type) ?? TRI_LOAD_TYPES[1]
              return (
                <div key={load.id} style={{ marginBottom:8, border:`1px solid ${llt.color}50`,
                  borderRadius:6, padding:'6px 8px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                    <span style={{ fontSize:10, fontWeight:700, color:llt.color }}>
                      {load.label} — {load.type} ({load.ligacao})
                    </span>
                    <button style={{ fontSize:13, color:'#dc2626', background:'none', border:'none', cursor:'pointer' }}
                      onClick={()=>removeLoad(load.id)}>×</button>
                  </div>
                  <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                    {llt.fields.map(f => (
                      <div key={f} style={{ display:'flex', alignItems:'center', gap:2 }}>
                        <span style={{ fontSize:9, color:'var(--c-text-muted)' }}>{f}=</span>
                        <input className="form-input" style={{ width:52, fontSize:10 }}
                          value={load[f] ?? ''} onChange={e=>updLoad(load.id,f,e.target.value)}/>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:12, minHeight:0 }}>
        <div className="panel" style={{ flexShrink:0 }}>
          <div className="panel__head">Diagrama Unifilar — Barramento</div>
          <div style={{ padding:'8px 12px', overflowX:'auto' }}>
            <BusDiagramSVG loads={result.loads} src={src}/>
          </div>
        </div>
        <div className="panel" style={{ flex:'1 1 auto', minHeight:180 }}>
          <div className="panel__head">Fasores — Equivalente por Fase (A=0°)</div>
          <TriPhasor3F
            VA_I={{ V: cx.polar(result.V_ph,0),            I: cx.polar(result.I_tot_ph, -result.phi_tot) }}
            VB_I={{ V: cx.polar(result.V_ph,-2*Math.PI/3), I: cx.polar(result.I_tot_ph, -2*Math.PI/3-result.phi_tot) }}
            VC_I={{ V: cx.polar(result.V_ph, 2*Math.PI/3), I: cx.polar(result.I_tot_ph,  2*Math.PI/3-result.phi_tot) }}
            V_ph={result.V_ph}
            inLabel={`FP_eq=${fmt(result.FP_tot,3)} · I_linha=${fmt(result.I_tot_ph,2)}A`}
          />
        </div>
      </div>
      ...
      <div className="panel" style={{ display:'flex', flexDirection:'column', minHeight:0 }}>
        <div className="panel__head">Resultados</div>
        <div style={{ flex:1, overflow:'auto', padding:'10px 12px' }}>
          <Section title="Total do Barramento">
            <Result label="I linha tot."  value={`${fmt(result.I_tot_ph,4)} A`}/>
            <Result label="FP total"      value={fmt(result.FP_tot,4)} highlight={result.FP_tot<0.92&&result.S_tot>0}/>
            <Result label="φ total"       value={`${fmt(result.phi_tot*180/Math.PI,2)}°`}/>
            <Result label="P total"       value={`${fmt(result.P_tot/1000,2)} kW`}/>
            <Result label="Q total"       value={`${fmt(result.Q_tot/1000,2)} kvar`}/>
            <Result label="S total"       value={`${fmt(result.S_tot/1000,2)} kVA`}/>
          </Section>
          {result.Rline > 0 && (
            <Section title="Linha">
              <Result label="Queda de V"  value={`${fmt(result.V_drop,1)} V (${fmt(result.vd_pct,2)}%)`} highlight={result.vd_pct>5}/>
              <Result label="V barramento" value={`${fmt(result.V_bus/1000,3)} kV`}/>
              <Result label="Perdas"       value={`${fmt(result.Ploss/1000,2)} kW`}/>
            </Section>
          )}
          {result.loads.map(l => (
            <Section key={l.id} title={`${l.label}`}>
              <Result label="I linha"  value={`${fmt(l.I_line,4)} A`}/>
              <Result label="FP"       value={fmt(l.FP,4)} highlight={l.FP<0.92&&l.S>0}/>
              <Result label="φ"        value={`${fmt(l.phi*180/Math.PI,2)}° (${l.nat})`}/>
              <Result label="P"        value={`${fmt(l.P/1000,3)} kW`}/>
              <Result label="Q"        value={`${fmt(l.Q/1000,3)} kvar`}/>
              <Result label="S"        value={`${fmt(l.S/1000,3)} kVA`}/>
            </Section>
          ))}
        </div>
      </div>
...
function SubCC() { return <CircuitBuilder isCA={false}/> }
function _SubCC_legacy() {
...
function SubCAMono() { return <CircuitBuilder isCA={true}/> }
function _SubCAMono_legacy() {
...
function SubCATri() {
  const [mode, setMode]     = useState('construtor') // 'construtor' | 'analise' | 'editor'
    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', flex:'1 1 auto', minHeight:0 }}>
        <div style={{ display:'flex', gap:6, padding:'6px 12px', background:'var(--c-surface)', borderBottom:'1px solid var(--c-border)' }}>
          <button className="btn btn-sm" style={{ background:'#1d4ed8', color:'#fff' }}>⚡ Construtor</button>
          <button className="btn btn-sm btn-ghost" onClick={()=>setMode('analise')}>Análise de Cargas</button>
          <button className="btn btn-sm btn-ghost" onClick={()=>setMode('editor')}>Editor Paramétrico</button>
        </div>
        <TriBusBuilder/>
      </div>
    )
        ...
        <div style={{ flex:1, overflow:'auto', padding:'10px 12px' }}>
          ...
          <div style={{ display:'flex', gap:6, marginBottom:10 }}>
            <button className="btn btn-ghost btn-sm" style={{ flex:1 }}
              onClick={()=>setMode('construtor')}>⚡ Construtor</button>
            <button className="btn btn-ghost btn-sm" style={{ flex:1 }}
              onClick={()=>setMode('editor')}>Editor →</button>
          </div>
